...
/**
 * Generate a Caleb University payment receipt (card-based design) piped to response.
 *
 * @param {res} Express response object
 * @param {object} data Payment receipt details
 */
const generateReceiptPDF = async (res, data) => {
  return new Promise((resolve, reject) => {
    ...
    res.setHeader('Content-Disposition', `attachment; filename=receipt_${data.transactionId || 'payment'}.pdf`);
    ...
    doc.font('DejaVu-Bold').fontSize(14).fillColor('#FFFFFF')
       .text(`\u20A6${fmtMoney(data.amount)}`, cardX + cardPadding + amountBoxPad + 80, ry + amountBoxPad - 2,
...
module.exports = { generateReceiptPDF, numToWords };
